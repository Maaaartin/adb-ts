...
export default class AdbClient extends EventEmitter {
    ...
    constructor(options?: AdbClientOptions) {
        ...
        this.options.noAutoStart = options.noAutoStart != undefined ? options.noAutoStart : false;
    }

    startServer(cb?: (err: Error) => void): Promise<void> {
        const port = this.options.port;
        const args = port ? ['-P', port.toString(), 'start-server'] : ['start-server'];
    ...
    transport(serial: string): Promise<Connection> {
        return this.connection()
            .then((conn) => {
                return new HostTransportCommand(conn)
                    .execute(serial).return(conn);
            })
    }
    ...
    version(cb?: (err: Error, value?: number) => void) {
        return this.connection()
            .then((conn) => {
                return new VersionCommand(conn)
                    .execute();
            }).nodeify(cb);
    }
    ...
    connect(host: string, port?: any, cb?: (err: Error, value: number) => void) {
        ...
        return this.connection()
            .then((conn) => {
                return new ConnectCommand(conn)
                    .execute(host, port);
            }).nodeify(cb);
    }
    ...
    disconnect(host: string, port?: any, cb?: (err: Error, value: number) => void) {
        if (typeof port === 'function') {
            cb = port;
            port = undefined;
        }
        ...
        return this.connection()
            .then((conn) => {
                return new DisconnectCommand(conn)
                    .execute(host, port);
            });
    }
    ...
    getDevicePath(serial: string, cb?: (err: Error, value: string) => void) {
        return this.connection()
            .then((conn) => {
                return new GetDevicePathCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }

    listProperties(serial: string, cb?: (err: Error, value: KeyStringObject) => void) {
        return this.connection()
            .then((conn) => {
                return new ListPropertiesCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }

    listFeatures(serial: string, cb?: (err: Error, value: KeyStringObject) => void) {
        return this.connection()
            .then((conn) => {
                return new ListFeaturesCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }
    ...
    listPackages(serial: string, cb?: (err: Error, value: string[]) => void) {
        return this.connection()
            .then((conn) => {
                return new ListPackagesCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }
    ...
    getIpAddress(serial: string, cb?: (err: Error, value: string) => void): Promise<string | undefined> {
        return this.connection()
            .then((conn) => {
                return new GetIpAddressCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }

    forward(serial: string, local: string, remote: string, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new ForwardCommand(conn)
                    .execute(serial, local, remote);
            }).nodeify(cb);
    }

    listForwards(serial: string, cb?: (err: Error, value: ForwardsObject[]) => void) {
        return this.connection()
            .then((conn) => {
                return new ListForwardsCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }

    reverse(serial: string, local: string, remote: string, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new ReverseCommand(conn)
                    .execute(serial, local, remote);
            }).nodeify(cb);
    }

    listReverses(serial: string, cb?: (err: Error, value: ReversesObject[]) => void) {
        return this.connection()
            .then((conn) => {
                return new ListReversesCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }
    ...
    private shellInternal(serial: string, command: string | string[]) {
        return this.connection()
            .then((conn) => {
                return new ShellRawCommand(conn)
    ...
    shutdown(serial: string, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new ShutdownCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }
    ...
    remount(serial: string, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new RemountCommand(conn)
                    .execute(serial);
            }).nodeify(cb);
    }
    ...
    openTcp(serial: string, port: number | string, host?: any, cb?: (err: Error, value: Connection) => void): Promise<any> {
    ...
    roll(serial: string, x: number, y: number, source: any, cb?: (err: Error) => void) {
        ...
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, source, 'roll', x, y);
            }).nodeify(cb);
    }
    ...
    press(serial: string, source: any, cb?: (err: Error) => void) {
        ...
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, source, 'press');
            }).nodeify(cb);
    }

    dragAndDrop(serial: string, x1: number, y1: number, x2: number, y2: number, cb?: (err: Error) => void): Promise<void>;
    dragAndDrop(serial: string, x1: number, y1: number, x2: number, y2: number, options?: InputOptions & { duration?: number }, cb?: (err: Error) => void): Promise<void>;
    dragAndDrop(serial: string, x1: number, y1: number, x2: number, y2: number, options?: any, cb?: (err: Error) => void) {
        ...
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, options.source, 'draganddrop', x1, y1, x2, y2, options.duration ? options.duration : '');
            }).nodeify(cb);
    }

    swipe(serial: string, x1: number, y1: number, x2: number, y2: number, cb?: (err: Error) => void): Promise<void>;
    swipe(serial: string, x1: number, y1: number, x2: number, y2: number, options?: InputOptions & { duration?: number }, cb?: (err: Error) => void): Promise<void>;
    swipe(serial: string, x1: number, y1: number, x2: number, y2: number, options?: any, cb?: (err: Error) => void) {
        if (typeof options === 'function' || !options) {
            cb = options;
            options = {};
        }
        ...
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, options.source, 'swipe', x1, y1, x2, y2, options.duration ? options.duration : '');
            }).nodeify(cb);
    }

    keyEvent(serial: string, code: KeyCode | number, cb?: (err: Error) => void): Promise<void>;
    keyEvent(serial: string, code: KeyCode | number, options?: InputOptions & { longpress?: boolean }, cb?: (err: Error) => void): Promise<void>;
    keyEvent(serial: string, code: KeyCode | number, options?: any, cb?: (err: Error) => void) {
        if (typeof options === 'function' || !options) {
            cb = options;
            options = {};
        }
        ...
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, 'keyevent', options.source, code, options.longpress ? '--longpress' : '');
            }).nodeify(cb);
    }
    ...
    tap(serial: string, x: number, y: number, source: any, cb?: (err: Error) => void) {
        if (typeof source === 'function') {
            cb = source;
        }
    ...
    text(serial: string, text: SimpleType, source: any, cb?: (err: Error) => void) {
        if (typeof source === 'function') {
            cb = source;
        }
        source = source || 'touchscreen';
        return this.connection()
            .then((conn) => {
                return new InputCommand(conn)
                    .execute(serial, source, 'text', text);
            }).nodeify(cb);
    }
    ...
    openLogcat(serial: string, options?: any, cb?: (err: Error, value: LogcatReader) => void) {
        ...
        return this.connection()
            .then((conn) => {
                return new LogcatCommand(conn)
                    ...
                    .then((stream) => {
                        const logCat = Logcat.readStrem(stream, { ...options, fixLineFeeds: false });
    ...
    clear(serial: string, pkg: string, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new ClearCommand(conn)
                    .execute(serial, pkg);
            }).nodeify(cb);
    }

    private installRemote(serial: string, apk: string, options?: InstallOptions, args?: string) {
        return this.connection()
            .then((conn) => {
                return new InstallCommand(conn)
                    .execute(serial, apk, options, args)
                    .then(() => {
                        return this.shellInternal(serial, ['rm', '-f', apk])
                            .then((stream) => {
    ...
    install(serial: string, apk: string | Readable, options?: any, args?: any, cb?: (err: Error) => void): Promise<void> {
        if (typeof options === 'function') {
            cb = options;
            options = undefined;
        }
        ...
        return this.push(serial, apk, temp)
            .then((transfer) => {
                let errorListener: ReadableStreamErrorCallback;
    ...
    uninstall(serial: string, pkg: string, options?: any, cb?: (err: Error) => void) {
        if (typeof options === 'function') {
            cb = options;
            options = undefined;
        }
        return this.connection()
            .then((conn) => {
                return new UninstallCommand(conn)
                    .execute(serial, pkg, options);
            }).nodeify(cb);
    }

    isInstalled(serial: string, pkg: string, cb?: (err: Error, value: boolean) => void) {
        return this.connection()
            .then((conn) => {
                return new IsInstalledCommand(conn)
                    .execute(serial, pkg);
            }).nodeify(cb);
    }
    ...
    startActivity(serial: string, pkg: string, activity: string, options?: any, cb?: (err: Error) => void) {
        if (typeof options === 'function') {
            cb = options;
            options = undefined;
        }
        return this.connection()
            .then((conn) => {
                return new StartActivityCommand(conn)
                    .execute(serial, pkg, activity, options);
            }).nodeify(cb);
    }

    startService(serial: string, pkg: string, service: string, cb?: (err: Error) => void)
    startService(serial: string, pkg: string, service: string, options?: StartServiceOptions, cb?: (err: Error) => void): Promise<void>;
    startService(serial: string, pkg: string, service: string, options?: any, cb?: (err: Error) => void) {
        if (typeof options === 'function') {
            cb = options;
            options = undefined;
        }
        return this.connection()
            .then((conn) => {
                return new StartServiceCommand(conn)
                    .execute(serial, pkg, service, options);
            }).nodeify(cb);
    }
    ...
    readDir(serial: string, path: string, cb?: (err: Error, value: SyncEntry[]) => void) {
        return this.syncService(serial)
            .then((sync) => {
    ...
    pull(serial: string, path: string, cb?: (err: Error, value: PullTransfer) => void) {
        return this.syncService(serial)
            .then((sync) => {
                return sync.pull(path)
                    .on('end', () => {
                        sync.end();
                    });
            }).nodeify(cb);
    ...
    push(serial: string, srcPath: string | Readable, destPath: string, mode?: any, cb?: (err: Error, value: PushTransfer) => void) {
        ...
        return this.syncService(serial)
            .then((sync) => {
                return sync.push(srcPath, destPath, mode)
                    .on('end', () => {
                        sync.end();
                    });
            }).nodeify(cb);
    ...
    tcpip(serial: string, port: any, cb?: (err: Error, value: string) => void) {
        ...
        return this.connection().then((conn) => {
            return new TcpIpCommand(conn)
                .execute(serial, port, this.options.host)
        }).nodeify(cb);
    }

    usb(serial: string, cb?: (err: Error) => void) {
        return this.connection().then((conn) => {
            return new UsbCommand(conn)
                .execute(serial)
        }).nodeify(cb);
    }
    ...
    waitBootComplete(serial: string, cb?: (err: Error) => void) {
        return this.connection().then((conn) => {
            return new WaitBootCompleteCommand(conn)
                .execute(serial)
        }).nodeify(cb);
    }

    waitForDevice(tranport: TransportType, state: WaitForState, cb?: (err: Error) => void) {
        return this.connection().then((conn) => {
            return new WaitForDeviceCommand(conn)
                .execute(tranport, state)
        }).nodeify(cb);
    }
    ...
    pushDataToFile(serial: string, data: string, destPath: string, cb?: (err: Error) => void) {
        return this.pushInternal(serial, stringToStreamfrom(data), destPath)
            .nodeify(cb);
    }

    pushFile(serial: string, srcPath: string, destPath: string, cb?: (err: Error) => void) {
    ...
    setProp(serial: string, prop: string, value: SimpleType, cb?: (err: Error) => void): Promise<void> {
        return this.connection()
            .then((conn) => {
                return new SetProp(conn)
                    .execute(serial, prop, value)
            }).nodeify(cb);
    }

    getProp(serial: string, prop: string, cb?: (err: Error, value: SimpleType) => void) {
        return this.connection()
            .then((conn) => {
                return new GetPropertyCommand(conn)
                    .execute(serial, prop)
            }).nodeify(cb);
    }


    putSetting(serial: string, mode: SettingsMode, name: string, value: SimpleType, cb?: (err: Error) => void) {
        return this.connection()
            .then((conn) => {
                return new PutSetting(conn)
                    .execute(serial, mode, name, value)
            }).nodeify(cb);
    }

    listSettings(serial: string, mode: SettingsMode, cb?: (err: Error, value: KeyStringObject[]) => void) {
        return this.connection()
            .then((conn) => {
                return new ListSettingsCommand(conn)
                    .execute(serial, mode)
            }).nodeify(cb);
    }

    getSetting(serial: string, mode: SettingsMode, name: string, cb?: (err: Error, value: SimpleType) => void) {
        return this.connection()
            .then((conn) => {
                return new GetSetting(conn)
                    .execute(serial, mode, name)
            }).nodeify(cb);

    }

    shell(serial: string, command: string | string[], cb?: (err: Error, value: SimpleType) => void) {
        return this.connection()
            .then((conn) => {
                return new ShellCommand(conn)
                    .execute(serial, command)
            }).nodeify(cb);
    }
    ...
    openMonkey(serial: string, cb?: (err: Error, value: Monkey) => void) {
        const tryConnect = (times: number): Promise<Monkey> => {
            return this.openTcp(serial, 1080).then(stream => {
        ...
        return tryConnect(1)
            .catch((err) => {
                return this.transport(serial).then((transport) => {
