...
export default class InstallCommand extends TransportCommand {
    ...
    execute(serial: string, apk: string, options?: InstallOptions, args?: string) {
        return super.execute(serial, `shell:pm install`, ...this.intentArgs(options), this.escapeCompat(apk), args || '')
            .then((reply) => {
                switch (reply) {
                    case Reply.OKAY:
